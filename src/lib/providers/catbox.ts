...
import type { DeleteResult, UploadOptions, UploadResult } from "@/types/file";
...
export class CatboxProvider implements StorageProvider {
  async uploadFile(file: File, options: UploadOptions): Promise<UploadResult> {
    ...
    const resolvedAcceptedTypes = acceptedTypes.flatMap(
      (typeKey) => FILE_TYPES[typeKey.toUpperCase() as keyof typeof FILE_TYPES],
    );
    ...
    if (!validateFileSize(file, maxSize)) {
      return {
        message: null,
        error: `File too large, max file size is ${maxSize}mb`,
        name: null,
      };
    }
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        const result = await ky
          .post<CatboxUploadResponse>(`/api/catbox`, {
            body: formData,
            timeout: 30000,
          })
          .json();
  ...
  async deleteFiles(urls: string[]): Promise<DeleteResult> {
    try {
      ...
      if (!env.CATBOX_API) throw new Error("Catbox API is missing");
