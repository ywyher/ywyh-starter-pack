...
export async function ensureAuthenticated() {
  try {
    ...
    if (!currentUser?.user.id && env.ALLOW_ANONYMOUS_USERS) {
      ...
      if (!anon?.user?.id) {
        return {
          ...
          isAnon: false,
          error:
            "Not authenticated nor were we able to authenticate you as an anonymous user. Please register.",
        };
      }
    ...
    return {
      ...
      error:
        error instanceof Error
          ? error.message
          : "Failed to ensure user authentication",
      message: null,
      isAnon: false,
    };
  }
